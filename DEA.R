...
setwd('~/')
setwd("C:/Users/beatr/OneDrive - FCT NOVA/Ambiente de Trabalho/Mestrado/2º semestre/ACM/Project") #Complete with the path in your computer
...
plotCol <- plotCol <- rep(c("steelblue", "steelblue", "steelblue", "gray", "gray", "gray", "gray", "gray", "gray"))
...
legend("topleft", fill=unique(plotCol), legend=c("Arid1bKD","Non_targeting"), bty="n")
...
legend("topright", fill=unique(plotCol), legend=c("Arid1bKD","Non_targeting"), bty="n")
...
design_matrix
contrast_matrix <- makeContrasts(Arid1bKD-Non_targeting, levels = design_matrix) # NOTE: REPPLACE HERE WITH THE NAMES OF YOUR SAMPLES
contrast_matrix
...
points(plotData[downReg,1],plotData[downReg, 2], col="steelblue", pch=20)
text(plotData[upReg[1:13],], labels=upReg[1:5],col="tomato", pos=sample(c(1:3), size=10, replace=T), cex=0.8)
text(plotData[downReg,1],plotData[downReg, 2], labels=downReg[1:1],col="steelblue", pos=sample(c(1:2), size=10, replace=T), cex=0.8)
#dev.off()
...
text(plotData[upReg[1:13],], labels=upReg[1:5],col="tomato", pos=sample(c(1:3), size=10, replace=T), cex=0.8)
text(plotData[downReg[1:13],], labels=downReg[1:1],col="steelblue", pos=sample(c(1:2), size=10, replace=T), cex=0.8)
...
heatmap.2(logTPMs[c(upReg[1:10], downReg[1:1]),], scale="row", trace="none", density.info="none", ColSideColors = plotCol, col=plotCol_exp)

write.xlsx(uniqueGenes, file = 'EG.XLSX', rowNames = T)
write.table(rownames(allGenes), file = 'EG.txt', row.names = F, quote = F, col.names = F )

write.table(rownames(DEGs), file = "DEGs.txt", row.names = F, quote = F, col.names = F)


allGenes$rank <- sign(allGenes$logFC)*-1*log10(allGenes$PValue)

ranks <- data.frame(rownames(allGenes))
ranks$rank <- allGenes$rank
head(ranks)

write.table(ranks, file = 'ranks.rnk', row.names = F, quote = F, col.names = F, sep = '\t')
